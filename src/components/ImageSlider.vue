<template>
  <div v-for="(feature, index) in features" :key="index">
    <div
      :class="[TheImageSlider.container, 'relative rounded-md overflow-hidden']"
    >
      <div class="bg-primary absolute top-0 right-0 bottom-0 left-0">
        <img
          class="w-full h-full aspect-video object-cover"
          :src="`btc-gift/assets/images/${feature.image}.jpg`"
          alt=""
        />
        <div
          class="
            absolute
            top-0
            right-0
            bottom-0
            left-0
            flex
            justify-between
            items-center
            bg-overlay
          "
        >
          <button class="icon-button p-4">
            <span class="icon-angle-right text-background"></span>
          </button>
          <button class="icon-button p-4">
            <span class="icon-angle-left text-background"></span>
          </button>
        </div>
        <div class="absolute right-0 bottom-0 left-0 w-full">
          <div></div>
        </div>
      </div>
    </div>
    <h2 class="mt-4 text-center">{{ feature.title || "بدون عنوان" }}</h2>
    <p class="mt-4 text-center">{{ feature.description || "بدون توضیحات" }}</p>
  </div>
</template>
<script lang="ts" setup>
import featureModel from "../models/feature.model";
interface Props {
  features: featureModel[];
}

const props = defineProps<Props>();
</script>
<style lang="scss" module="TheImageSlider">
.container {
  padding-top: 56%;
}
</style>